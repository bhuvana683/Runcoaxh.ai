...
async def generate_workout_badge(
    ...
    try:
        ...
        return {
            "status": "success",
async def generate_image_art(
    prompt: str,
    tool_context: ToolContext,
) -> dict:
    """Generate ANY requested AI image, graphic, workout illustration, meal photo, or art using Google's Imagen model.

    Saves the generated image as an artifact in ADK tool_context (so it appears in Playground Artifacts panel)
    and uploads the image bytes directly to the public Cloud Storage bucket, returning its public https URL.

    Args:
        prompt: Detailed description of the image, graphic, illustration, or artwork to generate (e.g. 'A futuristic high-tech gym with neon lights', 'A delicious high-protein avocado salmon meal prep bowl').
        tool_context: ADK ToolContext instance for saving session artifacts.
    ...
    try:
        response = client.models.generate_content(
            model="gemini-3.1-flash-lite-image",
            contents=f"A vibrant high quality image or illustration: {prompt}",
            config=types.GenerateContentConfig(
                response_modalities=["IMAGE"],
            ),
        )
        ...
        return {
            ...
            "message": f"Successfully generated AI image '{filename}'."
        }
    except Exception as e:
        return {"error": f"Failed to generate AI image: {str(e)}"}
