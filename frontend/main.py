...
def _get_resource_name() -> str:
    ...
    raise KeyError("AGENT_ENGINE_RESOURCE_NAME environment variable is not set")
...
def _extract_parts(parts: list) -> list[dict]:
    ...
    application/json+a2ui or holding A2UI dicts) become {"kind": "a2ui", "data": <message>}
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            if mime == _A2UI_MIME or (isinstance(root.data, dict) and any(k in root.data for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate"))):
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Fallback: pull parts from task.history for agent messages or final task's artifacts.
        if not got_artifact_update and last_task is not None:
            if getattr(last_task, "history", None):
                for m in last_task.history:
                    if getattr(m, "role", None) == Role.agent or getattr(m, "role", None) == "agent":
